      ...
      {loading ? (
        ...
        <Card className="bg-luxe-charcoal border-luxe-gray">
          <CardContent className="py-12 text-center text-luxe-silver">
            Keine Waves. Tabelle <code className="text-xs bg-luxe-black px-1 rounded">warehouse_ops.picking_waves</code> über Migration anlegen.
          </CardContent>
        </Card>
      ) : (
        <Card className="bg-luxe-charcoal border-luxe-gray">
          <CardHeader>
            <CardTitle className="text-white">Pick-Waves</CardTitle>
            ...
            <ul className="space-y-3 text-sm">
              {waves.map((w) => (
                <li key={w.wave_id} className="flex items-center justify-between py-3 border-b border-luxe-gray/50 last:border-0">
                  ...
                  <span className="text-luxe-silver">{w.total_items_to_pick} Items · {w.status}</span>
