 ...
 * Body: orderId oder order_number; optional trackingNumber+trackingCarrier (dann Sendung hinzufügen).
 * Body: action: "send-email" + orderId/order_number = nur E-Mail mit allen Sendungsnummern senden.
 */
  ...
  try {
    ...
    if (!hasSupabaseAdmin()) {
      return NextResponse.json(
        { error: 'Sendungsnummer kann nicht gespeichert werden: SUPABASE_SERVICE_ROLE_KEY fehlt.' },
        { status: 503 }
      )
    }
    ...
    if (orderIdFromBody) {
      const res = await admin
        ...
        .maybeSingle()
      if (res.error) {
        console.error('Tracking (body): Admin byId error', { orderId: orderIdFromBody, error: res.error.message, code: res.error.code })
      }
    ...
    if (!order && orderNumberFromBody) {
      const res = await admin
        .from('orders')
        .select('id, order_number, customer_name, customer_email, status, tracking_number')
        .eq('order_number', orderNumberFromBody)
        .maybeSingle()
      if (res.error) console.error('Tracking (body): Admin byOrderNumber error', { order_number: orderNumberFromBody, error: res.error.message })
      if (res.data) order = res.data as OrderRow
    }
    if (!order && orderNumberFromBody) {
      const res = await admin
        .from('orders')
        .select('id, order_number, customer_name, customer_email, status, tracking_number')
        .ilike('order_number', orderNumberFromBody)
        .maybeSingle()
      if (res.data) order = res.data as OrderRow
    }

    if (!order) {
      console.warn('Tracking: Bestellung nicht gefunden', { orderIdFromBody: orderIdFromBody || '(leer)', orderNumberFromBody: orderNumberFromBody || '(leer)' })
      const payload: { error: string; debug?: string } = {
        error: 'Bestellung nicht gefunden. Bitte Bestell-ID oder Bestellnummer prüfen (z. B. aus der Bestellübersicht).',
      }
